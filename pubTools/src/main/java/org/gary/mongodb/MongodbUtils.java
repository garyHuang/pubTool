...
public class MongodbUtils {
	 ...
	 * */
	public MongodbUtils(String dbName, String host, Integer port, String user,
			String pwd) {
		this.host = host;
		this.dbName = dbName;
		this.port = port;
		...
		for (String key : objs.keySet()) {
			...
			if (null != val) {
				if(val instanceof BigDecimal){
					val = TransformUtils.toDouble( val );
				}
	...
	public void delete(String tName, Bson bson){
		getMongoColl(tName);
		try {
			mongoColl.deleteMany( bson ) ;
		} catch (Exception e) {
			LogManager.err("删除出现错误", e);
		}
	}
	/**
	 * 通用查询方法
	...
	public void update(String tName, Map<String, Object> data, String updateKey) {
		...
		if (_ID.equalsIgnoreCase(updateKey)) {
			filter.append(updateKey,
					new ObjectId(String.valueOf(data.get(updateKey))));
		} else {
